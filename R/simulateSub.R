...
##' @rdname simulateSub-methods
##' @export 
setGeneric( "simulateSub",
            function( object, ... )
              standardGeneric( "simulateSub")
)
...
##' @rdname simulateSub-methods
##' @export
setMethod( "simulateSub",signature=c( "EventModelSubgroup" ),
           function( object, ... ){
             simulate.Subgroup( object@event.data, object@s1, object@s2, 
                                object@simParams.s1, object@simParams.s2, ... )
           })

##' Only used internally
##' @param data EventData object
##' @param s1 Name (label) of subgroup 1
##' @param s2 Name (label) of subgroup 2
##' @param SimParams.s1 Parameters for fitted for first subgroup
##' @param SimParams.s2 Parameters for fitted for second subgroup
...
simulate.Subgroup <- function( data, 
                               s1, s2,
                               SimParams.s1, SimParams.s2,
                               Nsim, seed,
                               accrualGenerator=NULL, Naccrual=0, 
                               limit=0.05, longlagsettings=NULL,
                               dropout=NULL ){
  ...
  simParams.s1 <- SimParams.s1@generateParameterFunction( Nsim )
  simParams.s2 <- SimParams.s2@generateParameterFunction( Nsim )
  indat$isS1 <- as.integer( indat$subgroup==s1 )
  simParams <- cbind( simParams.s1[ , -1 ], simParams.s2[ , -1] )
  colnames( simParams ) <- c( "rate.s1", "shape.s1", "rate.s2", "shape.s2" ) 
  #perform the simulations    
  outcomes <-apply( simParams, 1, PerformOneSubgroupSimulation,
                    number.subjects=nrow(indat),
                    Naccrual=Naccrual,
                    indat=indat,newrecs=newrecs,HR=HR,r=r,
                    dropout.rate=dropout.rate, 
                    dropout.shape=dropout.shape, followup=data@followup,
                    conditionalFunction.s1=SimParams.s1@conditionalFunction,
                    conditionalFunction.s2=SimParams.s2@conditionalFunction )
...
# @param conditionalFunction.s1 The conditionalFunction of the first subgroup, from a \code{FromDataSimParam} object 
# @param conditionalFunction.s2 The conditionalFunction of the second subgroup, from a \code{FromDataSimParam} object 
...
PerformOneSubgroupSimulation <- function(row,
                                         number.subjects,Naccrual,
                                         indat, newrecs,HR,r, dropout.rate, 
                                         dropout.shape, followup,
                                         conditionalFunction.s1, conditionalFunction.s2 ){
  ...
  row.1 <- data.frame( shape = row$shape.s1, rate = row$rate.s1 )
  times.1 <- conditionalFunction.s1(t.conditional=lower.bounds.1,params=row.1,HR=HR.1) 
  ...
  row.2 <- data.frame( shape = row$shape.s2, rate = row$rate.s2 )
  times.2 <- conditionalFunction.s2(t.conditional=lower.bounds.2,params=row.2,HR=HR.2) 
